...
class theme_modern_core_renderer extends core_renderer {
    ...
    protected function mycourses($CFG,$sidebar){
        ...
        for($x=1;$x<=sizeof($courselist);$x++){
            ...
            $url = $CFG->wwwroot."/theme/modern/pix/coursenoimage.jpg";
    ...
    protected function render_user_menu(custom_menu $menu) {
        global $CFG, $USER, $DB, $PAGE; //Modern add $PAGE
        ...
        if ($addusermenu) {
            if (isloggedin() && !isguestuser()) {
                ...
                if (!empty($PAGE->theme->settings->enablemy)) {
                    $usermenu->add(
                        '<i class="fa fa-briefcase"></i>' . get_string('mydashboard','theme_modern'),
                        new moodle_url('/my', array('id'=>$USER->id)),
                        get_string('mydashboard','theme_modern')
                    );
                }
   ...
   protected function process_user_messages() {
    ...
    protected function get_user_messages() {
        ...
        foreach ($newmessages as $message) {
            $messagelist[] = $this->theme_modern_process_message($message);
        }
   ...
   protected function theme_modern_process_message($message) {
        global $DB;
   ...
   protected function render_tabobject(tabobject $tab) {
    ...
    public function navbar_button_reader($dataid = '#region-main', $class = null) {
        ...
        $content = html_writer::link('#', $icon . ' ' . get_string('reader','theme_modern'),
...
function theme_modern_get_nav_links($course, $sections, $sectionno) {
        ...
        while ($back > 0 and empty($links['previous'])) {
            if ($canviewhidden || $sections[$back]->uservisible) {
                ...
                $previouslink .= get_string('previoussection', 'theme_modern');
        ...
        while ($forward <= $course->numsections and empty($links['next'])) {
            if ($canviewhidden || $sections[$forward]->uservisible) {
                ...
                $nextlink .= get_string('nextsection', 'theme_modern');
...
class theme_modern_format_topics_renderer extends format_topics_renderer {

    protected function get_nav_links($course, $sections, $sectionno) {
        return theme_modern_get_nav_links($course, $sections, $sectionno);
    }
...
class theme_modern_format_weeks_renderer extends format_weeks_renderer {
...
if (file_exists("$CFG->dirroot/course/format/topcoll/renderer.php")) {
    ...
    class theme_modern_format_topcoll_renderer extends format_topcoll_renderer {
...
if (file_exists("$CFG->dirroot/course/format/grid/renderer.php")) {
    ...
    class theme_modern_format_grid_renderer extends format_grid_renderer {
...
if (file_exists("$CFG->dirroot/course/format/noticebd/renderer.php")) {
    ...
    class theme_modern_format_noticebd_renderer extends format_noticebd_renderer {
...
if (file_exists("$CFG->dirroot/course/format/columns/renderer.php")) {
    ...
    class theme_modern_format_columns_renderer extends format_columns_renderer {
